...
def get_timezone():
    user_preferred_timezone = request.args.get('timezone', None)
    user = g.user
    if user:
        return users["timezone"]
    if user_preferred_timezone and user_preferred_timezone in Config.LANGUAGES:
        return user_preferred_timezone
